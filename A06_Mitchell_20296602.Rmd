...
runtime: shiny
---
##Biol 432: Assignmnet 6 - High Performance Computing## 
Emma Mitchell - 20296602

Git Hub: 
https://github.com/emmaamitchyy/biol432
...
There are 8 cores in this system. This means that there are 8 individual processing units within the CPU, all of which excute instructions and preform the calculations. 
...
The results from these calculations show that the serial loop's runtime was ~20 secs, the theoretical pparallel loops time was ~3 secs and the actual parallel time was ~6.5 secs. The parallel program significantly reduces the time it takes to run the loop. The actual parallel run time was longer than the predicted theoretical runtime by 3 seconds. This can happen due to operational "costs" associated with parallel execution. These "costs" could be the distribution of the data (the tasks splitting among cores), the processing time it takes to manage the memory, and the combination of the results from each core. Although the parallel loop takes longer than the theoretical speed, it is still much faster than serial execution. This shows that it is very efficient compute in parallel, especially for large datasets, 
